...
export async function selectCharacterById(id, { switchMenu = true } = {}) {
    await selectCharacterByIdCore(id, { switchMenu });
...
export function getCharacterCardFields({ chid = null } = {}) {
    return getCharacterCardFieldsCore({ chid });
}
...
async function read_avatar_load(input) {
    await readAvatarLoadCore(input);
    crop_data = characterCropData;
    syncCropData(crop_data);
}
